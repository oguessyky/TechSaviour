<?php
    ...
    if ($maxScore = $dbConn -> query("SELECT MAX(CPUScore) AS CPU, MAX(GPUScore) AS GPU, MAX(RAM) AS RAM, MAX(Storage) AS Storage FROM Laptop;") -> fetch_assoc()) {
        $maxCPUScore = $maxScore['CPU'];
        ...
        if (isset($_GET['category']) && in_array($category = $_GET['category'],["gaming","business","art"])) {
            ...
            $condition = "For".ucfirst($category);
            ...
            echo "<script>";
            $condition = "1";
            if (isset($_GET['cpuRating']) && is_int($cpuRating = json_decode($_GET['cpuRating']))) {
                $cpuRating = $cpuRating <= 10 ? $cpuRating : 10;
            ...
            if (isset($_GET['gpuRating']) && is_int($gpuRating = json_decode($_GET['gpuRating']))) {
                $gpuRating = $gpuRating <= 10 ? $gpuRating : 10;
            ...
            if (isset($_GET['ramRating']) && is_int($ramRating = json_decode($_GET['ramRating']))) {
                $ramRating = $ramRating <= 8 ? $ramRating : 8;
            ...
            if (isset($_GET['storageRating']) && is_int($storageRating = json_decode($_GET['storageRating']))) {
                $storageRating = $storageRating <= 8 ? $storageRating : 8;
        ...
        $result = $dbConn -> query("SELECT ID,Name,ImageAddress,CPUScore,GPUScore,Ram,Storage FROM Laptop WHERE $condition;");
                ...
                <div class='all-content'>
                    ...
                    <div class='component-detail'>
                        ...
                        <div class='component-cpu'>
                            ...
                            <progress class='cpu' value='".$row['CPUScore']."' max='$maxCPUScore'></progress>
                        </div>
                        <div class='component-gpu'>
                            ...
                            <progress class='gpu' value='".$row['GPUScore']."' max='$maxGPUScore'></progress>
                        </div>
                        <div class='component-ram'>
                            ...
                            <progress class='ram' value='".$row['Ram']."' max='$maxRAMScore'></progress>
                        </div>
                        <div class='component-ssd'>
                            ...
                            <progress class='ssd' value='".$row['Storage']."' max='$maxStorageScore'></progress>
                        </div>
                    </div>
                </div>
            </a>";
